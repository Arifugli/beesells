...
import { api, type User, type KpiCategory, type KpiTarget } from "@/lib/api";
...
type OperatorWithBranch = User & { branchId: number };

function TargetsModal({
  operator, categories, month, onClose,
}: {
  operator: User; categories: KpiCategory[]; month: string; onClose: () => void;
}) {
  ...
  return (
    <Modal open onClose={onClose} title={`Планы KPI — ${operator.name}`} maxWidth="max-w-lg">
      <p className="text-sm text-gray-500 mb-5">{formatMonth(month)}</p>
      {isLoading ? (
...
export default function ManagerOperators() {
  ...
  const opsByBranch = branches.map(b => ({
    branch: b,
    operators: (operators as OperatorWithBranch[]).filter(o => o.branchId === b.id),
  }));
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      {/* Targets Modal */}
      {targetsOp && (
        <TargetsModal operator={targetsOp} categories={categories} month={month} onClose={() => setTargetsOp(null)} />
      )}
