...
export default function ManagerOperators() {
  ...
  const openAdd = () => { setEditingOp(null); setFormName(""); setFormEmail(""); setFormPassword(""); setFormBranch(branches[0]?.id.toString() ?? ""); setFormOpen(true); };
  const openEdit = (op: OperatorWithBranch) => { setEditingOp(op); setFormName(op.name); setFormEmail(op.email ?? ""); setFormPassword(""); setFormBranch(op.branchId.toString()); setFormOpen(true); };
  ...
  const handleSave = () => {
    ...
    if (editingOp) {
      ...
      createMutation.mutate({ name: formName.trim(), branchId: Number(formBranch), email: formEmail.trim() || undefined, password: formPassword || undefined }, {
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      <Modal
        ...
        title={editingOp ? "Редактировать оператора" : "Добавить оператора"}
        footer={<>
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input type="text" value={formName} onChange={e => setFormName(e.target.value)}
              placeholder="Введите имя" className="input" autoFocus />
          </div>
          {!editingOp && (
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            <label className="text-sm font-medium">Email <span className="text-gray-400 font-normal">(для восстановления пароля)</span></label>
            <input type="email" value={formEmail} onChange={e => setFormEmail(e.target.value)}
              placeholder="operator@company.com" className="input" />
          </div>
          <div className="space-y-1.5">
            <label className="text-sm font-medium">
              {editingOp ? "Новый пароль" : "Пароль"} <span className="text-gray-400 font-normal">{editingOp ? "(оставьте пустым чтобы не менять)" : "(минимум 6 символов)"}</span>
            </label>
            <input type="password" value={formPassword} onChange={e => setFormPassword(e.target.value)}
              placeholder={editingOp ? "Введите новый пароль" : "Введите пароль"} className="input" />
          </div>
        </div>
      </Modal>
