...
export default function AdminManagers() {
  ...
  const openAdd = () => { setEditingId(null); setFormName(""); setFormEmail(""); setFormPassword(""); setFormOpen(true); };
  const openEdit = (m: any) => { setEditingId(m.id); setFormName(m.name); setFormEmail(m.email ?? ""); setFormPassword(""); setFormOpen(true); };
  ...
  const handleSave = () => {
    ...
    if (editingId) {
      ...
      createMutation.mutate({ name: formName.trim(), email: formEmail.trim() || undefined, password: formPassword || undefined }, {
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      <Modal
        ...
        title={editingId ? "Редактировать менеджера" : "Новый менеджер"}
        footer={<>
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input type="text" value={formName} onChange={e => setFormName(e.target.value)}
              placeholder="Умид Хасанов" className="input" autoFocus />
          </div>
          <div className="space-y-1.5">
            ...
            <input type="email" value={formEmail} onChange={e => setFormEmail(e.target.value)}
              placeholder="manager@company.com" className="input" />
          </div>
          <div className="space-y-1.5">
            <label className="text-sm font-medium">
              {editingId ? "Новый пароль" : "Пароль"}{" "}
              <span className="text-gray-400 font-normal">{editingId ? "(оставьте пустым чтобы не менять)" : "(минимум 6 символов)"}</span>
            </label>
            <input type="password" value={formPassword} onChange={e => setFormPassword(e.target.value)}
              placeholder={editingId ? "Новый пароль" : "Введите пароль"} className="input" />
          </div>
        </div>
      </Modal>
